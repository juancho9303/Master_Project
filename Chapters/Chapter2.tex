...
As displayed in figure [], the difference between the real (unlensed) angular position $\vec{\beta}$  and the observed position $\vec{\theta}$ is this deflection angle, reduced by a ratio of distances, described as the lens equation

\begin{equation}
\vec{\beta}=\vec{\theta}-\vec{\alpha}\left(\vec{\theta}\right)=\vec{\theta}-\frac{D_{ds}}{D_{s}}\vec{\hat{\alpha}}\left(\vec{D_{d}\theta}\right)
\end{equation}

where $D_{ds}$ is the distance from the lense to the sourse $D_s$ is the distance from the observer to the source and $D_d$ is the fistance from the observer to the source
...
\caption[Angles in gravitational lensing]{Wikipedia}
\end{figure}
...
A very useful parameter is the lensing jacobian defined as:

\begin{equation}
A_{ij}=\frac{\partial\beta_{i}}{\partial\theta_{j}}=\delta_{ij}-\frac{\partial\alpha_{i}}{\partial\theta_{j}}=\delta_{ij}-\frac{\partial^{2}\psi}{\partial\theta_{i}\partial\theta_{j}}
\end{equation}
