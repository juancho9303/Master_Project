...
In order to extract the same objects and make the segmentation masks for the desired objects in the different filters, we used \texttt{SEXTRACTOR} on dual mode and made aperture photometri on each of the relevant objects. Figure [4.6] shows the color magnitude diagram for ABELL754 where we used a zero point magnitude of 30.
...
We can also descriminate the stars from galaxies using the radius that contains most of the flux. Figure [4.7] shows the galaxies and stars in the Mag vs \texttt{Flux\_rad} plane.
