...
\end{tabular}
\caption[Abell Clusters and their redshift]{Abell clusters used in this work. Marked with * the chosen clusters with the most promising features. From left to right: Name of the cluster, redshift, velocity disperions from C. Bildfell et. al. \citeyear{Reference6}, distance in Mpc and Einstein Ring using a single isothermal sphere as first approximation.}
\end{table}
...
Some of the useful parameters that we used to properly fit the BCG in every case were:

Bending modes:
\texttt{B1)}  0.07      1        Bending mode 1 (shear)
\texttt{B2)}  0.01      1        Bending mode 2 (banana shape)
\texttt{B3)}  0.03      1        Bending mode 3 (S-shape)

Azimuthal fourier modes
\texttt{F1)}  0.07  30.1  1  1   Az. Fourier mode 1, amplitude and phase angle
\texttt{F2)}  0.01  10.5  1  1   Az. Fourier mode 2, amplitude and phase angle
\texttt{F6)}  0.03  10.5  1  1  Az. Fourier mode 6, amplitude and phase angle
\texttt{F10)}  0.08  20.5  1  1   Az. Fourier mode 10, amplitude and phase angle
\texttt{F20)}  0.01  23.5  1  1   Az. Fourier mode 20, amplitude and phase angle

Traditional Diskyness/Boxyness parameter c
\texttt{C0)} 0.1         0       traditional diskyness(-)/boxyness(+)
...
We use \texttt{IRAF} to make the color images using our g,r,u,i bands. Let's use ABELL754 which is a low redshift galaxy cluster with a calculated mass of $\text{M}_{200}=9.8\times 10^{15} \text{M}_{\odot}$ (Sifon et. al. \citeyear{Reference9})
...
Color magnitude diagram for ABELL1068
...
\caption[Color Magnitude diagram of ABELL1068]{Color Magnitude diagram of ABELL1068 with the differentiation of stars from galaxies}
\end{figure}
...
\caption[Magnitude vs Flux radius of ABELL1068]{Magnitude vs Flux radius of ABELL1068 to identify the galaxies using the criteria of their flux distribution}
\end{figure}

We use the COSMOS2015 (Laigle et. al \citeyear{Reference21}.) catalogue that contains photometric redshift of over half million galaxies in multiple bands to put another constraint in our study.
...
To measure the photometric redshift of the galaxies (after filtering out the stars) in the inner region of the cluster after the subtraction of the BCG, we use the photometric redshift code \texttt{EAZY} (Brammer et. al 2008) which uses an extensive collection of spectral energy distributions for galaxies in the range $0<z<4$. Fortunately, the code includes library from CFHT in the I and U bands but doesn't have the filters in the G and R bands so I used the SUBARU survey filter information to be able to compute the photometric redshifts using four bands.
