...
sigma_0_1  = 9.0692e9;                                 # Critical surface mass density for z=1
#sigma_0.2 = ; sigma_0.3 = ; sigma_0.4 = ; sigma_0.5 = ; sigma_0.6 = ; sigma_0.7 = ;\
#sigma_0.8 = ; sigma_0.9 = ;

x = np.arange(0.001,0.99,0.0001)
plt.plot(x*r_s,((r_s*delta_c*rho_c)/(sigma_c))*( ((8.*np.arctanh(np.sqrt((1-x)/(1+x))))/(x*x*np.sqrt(1-x*x))) +\
               ((4.*(np.log(x/2.)))/(x*x)) - (2./(x*x-1)) + ((4.*np.arctanh(np.sqrt((1-x)/(1+x))))/((x*x-1)*(1-x*x)**0.5)))\
               , c='blue', lw=1.5)
x = 1
plt.plot(x*r_s,((r_s*delta_c*rho_c)/(sigma_c))*(10./3.+4.*np.log(0.5)))
x = np.arange(1.0001,10,0.0001)
plt.plot(x*r_s,((r_s*delta_c*rho_c)/(sigma_c))*( ((8.*np.arctan(np.sqrt((x-1)/(1+x))))/(x*x*np.sqrt(x*x-1))) +\
               ((4*(np.log(x/2)))/(x*x)) - (2/(x*x-1)) + ((4*np.arctan(np.sqrt((x-1)/(1+x))))/((x*x-1)**1.5))),\
                label='$\mathrm{z = 1}$', c='blue', lw=1.5)
