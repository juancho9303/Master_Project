...
for i in range(len(sigmas)):
	...
	gamma_1 = ((r_s*delta_c*rho_c)/sigmas[i]*( ((8.*np.arctanh(np.sqrt((1-r/r_s)/(1+r/r_s))))/((r/r_s)**2*np.sqrt(1-(r/r_s)**2))) +\
	               ((4.*(np.log((r/r_s)/2.)))/((r/r_s)**2)) - (2./((r/r_s)**2-1)) + ((4.*np.arctanh(np.sqrt((1-(r/r_s))/(1+(r/r_s)))))/\
	               (((r/r_s)**2-1)*(1-(r/r_s)**2)**0.5))))
	sigma_1 = (( 2.0*r_s*delta_c*rho_c )/( (r/r_s)**2 - 1.0 )*( 1.0 -\
	...
	r = r_s
	gamma_2 =  ((r_s*delta_c*rho_c)/sigmas[i]*(10./3.+4.*np.log(0.5)))

	sigma_2 = (2.0*r_s*delta_c*rho_c/3.0)
	plt.plot(np.log10(r),1./((1.-(sigma_2/sigmas[i]))**2.-(gamma_2)**2.))

	r = np.arange(101,10000,0.01)
	gamma_3 = (((r_s*delta_c*rho_c))/sigmas[i]*( ((8.*np.arctan(np.sqrt(((r/r_s)-1)/(1+(r/r_s)))))/((r/r_s)**2*np.sqrt((r/r_s)**2-1))) +\
		  ((4*(np.log((r/r_s)/2.)))/((r/r_s)**2.)) - (2./((r/r_s)**2-1.)) + ((4*np.arctan(np.sqrt(((r/r_s)-1)/(1+(r/r_s)))))/(((r/r_s)**2-1)**1.5))))

	sigma_3 = (( 2.0*r_s*delta_c*rho_c )/( (r/r_s)**2. - 1.0 ) *( 1.0 - 2.0/( np.sqrt( (r/r_s)**2 - 1.0 ) )*\
