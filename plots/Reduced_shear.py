...
for i in range(len(sigmas)):
	...
	plt.plot(np.log10(r),((r_s*delta_c*rho_c)*( ((8.*np.arctanh(np.sqrt((1-r/r_s)/(1+r/r_s))))/((r/r_s)**2*np.sqrt(1-(r/r_s)**2))) +\
	...
	r = r_s
	plt.plot(np.log10(r),((r_s*delta_c*rho_c)*(10./3.+4.*np.log(0.5)))/(sigmas[i]-2.0*r_s*delta_c*rho_c/3.0)) 
	r = np.arange(101,10000,0.01)
	plt.plot(np.log10(r),(((r_s*delta_c*rho_c))*( ((8.*np.arctan(np.sqrt(((r/r_s)-1)/(1+(r/r_s)))))/((r/r_s)**2*np.sqrt((r/r_s)**2-1))) +\
