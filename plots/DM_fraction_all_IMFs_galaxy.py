...
c          = 7.9428                                      # Concentration parameter (Paper Dutton and Maccio) 
...
for i in range(len(R)):
   ...
   n_array_plus.append((1.3*N(R[i]))) 
   n_array_minus.append((0.7*N(R[i])))
...
ax0.set_xlim([0.1, 10])
line0, = ax0.loglog((R), (f_array1), '-.', c='darkorange', label = r'$\mathrm{Salpeter}$', lw = 2)
line0, = ax0.loglog((R), (f_array2), '--', c='darkred', label = r'$\mathrm{Kroupa}$', lw = 2)
line0, = ax0.loglog((R), (f_array3), '-.', c='darkgreen', label = r'$\mathrm{Chabrier}$', lw = 2)
line0, = ax0.loglog((R), (n_array), '-', c='black', label = r'$\mathrm{NFW\:Mass\:Profile}$', lw = 2)
#plt.fill_between(R, 0.3*n_array, 0.7*n_array, alpha=0.2)
plt.fill_between(R, n_array_minus, n_array_plus, alpha=0.2)
plt.ylabel(r'$\mathrm{M(r)/M_{\odot}}$',fontsize=18)
plt.legend(frameon=False,bbox_to_anchor=(0.95, 0.45), loc=1, borderaxespad=0.,prop={'size':13})

ax1 = plt.subplot(gs[1], sharex = ax0)
ax0.set_xlim([0.1, 10])
line1, = ax1.plot((R), (fractionDM1), '-.', c='darkorange', label = r'$\mathrm{M_{NFW}/M_{\star}\:Salpeter}$', lw = 2)
line1, = ax1.plot((R), (fractionDM2), '--', c='darkred', label = r'$\mathrm{M_{NFW}/M_{\star}\:Kroupa}$', lw = 2)
line1, = ax1.plot((R), (fractionDM3), ':', c='darkgreen', label = r'$\mathrm{M_{NFW}/M_{\star}\:Chabrier}$', lw = 2)
...
plt.savefig("DM_fraction_all_IMFs_galaxy.png")
plt.show()
