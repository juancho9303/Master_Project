...
I_e      = 1*10**6     # Effective brightness (I(Re))
...
plt.plot(x3,2*((2*r_s*delta_c*rho_c)/((x3*x3)-1))*(1-(2)/(np.sqrt((x3*x3)-1))*np.arctanh(np.sqrt((x3-1)/(1+x3)))))

#Mass density profile from mass to light ratio and de Vaucouleurs profile:
x = np.arange(0.1,10000,0.001)
plt.plot(x/r_s,4*I_e*np.exp(-b*(((x/r_s)/R_e)**(0.25)-1)))  #4*I(R) where I(R) is de Vaucouleurs
