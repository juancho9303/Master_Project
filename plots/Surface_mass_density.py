...
I_e = 10**7               # Effective Brightness mu=23.772
...
plt.plot(np.log10(x1*r_s),np.log10(((2*r_s*delta_c*rho_c)/((x1*x1)-1))*( 1. - 2./(np.sqrt(1-(x1*x1)))*np.arctanh(np.sqrt((1-x1)/(1+x1))))))
x2 = 1
plt.plot(np.log10(x2*r_s),np.log10((2*r_s*delta_c*rho_c)/3.))
x3 = np.arange(1.0001,100.,0.0001)
plt.plot(np.log10(x3*r_s),np.log10(((2*r_s*delta_c*rho_c)/((x3*x3)-1))*(1. - 2./(np.sqrt((x3*x3)-1))*np.arctan(np.sqrt((x3-1)/(1+x3))))),label='NFW profile')

# Mass density profile from M/L ratio and de Vaucouleurs profile:

R = np.arange(0.1,10000.,0.001)
plt.plot(np.log10(R),np.log10(4.*I_e*np.exp( -b*( ( R/R_e )**0.25 - 1. ) )),label='Stellar Surface Mass')  #4*I(R) where I(R) is de Vaucouleurs
...
I1, I1err = quad(surf_bright, 0.001, 10000.)
